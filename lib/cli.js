...
var cli = module.exports = function () {
    ...
    scripts.forEach(function (script) {
        ...
        if (scriptName === 'index') {
            return scriptOrder = require(script);
        }
        var scriptImport = require(script);
        data[scriptName] = dataGenerator(scriptImport, cli.options.number);
    });
    ...
    if(cli.options.fullOutput) {
        console.log(JSON.stringify(scriptOrder, null, 2) + ',');
